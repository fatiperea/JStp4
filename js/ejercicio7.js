class Contacto {
  ...
  set cargaNombre(nombre) {
    this.nombre = nombre;
  }
  set cargaTelefono(tel) {
    this.telefono = tel;
  }
}
...
class agendaContactos extends Contacto {
  ...
  listaContacto() {
    for (let i = 0; i < this.length; i++) {
      document.write(
        "Contacto: " + this[i].verNombre + " - " + this[i].verTelefono + "</br>"
      );
    }
  }
  buscaContacto(nombre1) {
    if (this.verNombre === nombre1) return this.verTelefono;
    else alert("No hay un contacto de nombre " + nombre1);
  }
  eliminaContacto(contacto1) {
    if (contacto1.existeContacto) this.splice();
  }
  ...
  agendaLlena() {
    ...
    if (this.length >= 10) {
      ...
      alert("Aun tiene espacio en la agenda");
  ...
  huecosLibres() {
    if (this.agendaLlena()) alert("No hay espacio libre");
    else {
      let espacioDispo = 0;
      i = 0;
      while (i <= this.length) {
        if (this[i] !== undefined) espacioDispo++;
        i++;
      }
      return espacioDispo;
    }
...
const agenda = new agendaContactos[(contacto1, contacto2)](); //new agendaContactos;//[{nombre: nombre1, telefono: tel1},{nombre: nombre2, telefono: tel2}];
